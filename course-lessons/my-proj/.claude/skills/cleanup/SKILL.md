...
1. Make sure that the history in @context/current-feature.md is in order from oldest to newest. Make sure that the timestamps are not made up and related to the actual time the file is updated rounded down to the nearest five minutes. Previous records can be the same time but never later that the current record timestand
